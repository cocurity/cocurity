...
import { getFixOrders, updateFixOrderStatus, type CocurityFixOrder } from "@/lib/client/fix-orders";
  ...
  return (
    <main className="space-y-6">
      <section className="co-noise-card rounded-2xl p-6">
        ...
        <p className="mt-2 text-sm text-slate-300">Track Cocurity Fix requests and your report history.</p>
      </section>

      <section className="co-noise-card rounded-2xl p-6">
        <h2 className="text-lg font-semibold text-slate-100">My Cocurity Fix</h2>
        {fixOrders.length === 0 ? (
            ...
            {fixOrders.map((item) => (
              <li key={item.id} className="rounded-xl border border-white/10 bg-white/5 px-4 py-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="text-sm font-semibold text-slate-100">{item.repoUrl || `Scan ${item.scanId}`}</p>
                  <span className="text-xs text-cyan-200">{statusLabel(item.status)}</span>
                </div>
                <p className="mt-1 text-xs text-slate-400">{formatDate(item.createdAt)}</p>
                <p className="mt-1 text-xs text-slate-300">Email: {item.email || "not provided"}</p>
